...
final class UserTests: XCTestCase {
    ...
    func testGettingAUsersAcronymsFromTheAPI() throws {
        ...
        _ = try Acronym.create(
            short: "LOL",
            long: "Laugh Out Loud", user: user,
            on: app.db
        )

        try app.test(.GET, "\(usersURI)\(user.id!)/acronyms", afterResponse: { res in
            let acronyms = try res.content.decode([Acronym].self)
